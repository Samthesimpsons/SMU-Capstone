...
from src.profile_coherence.discordance import annotate_transactions_with_discordance
...
DEFAULT_OUTPUT_DIRECTORY = Path("outputs/eda/profile_coherence")
...
def _build_volatility_only_risk_classes(
    ...
    """Pure volatility-quartile mapping over the full asset universe (sensitivity baseline).

    Used to quantify how much the headline numbers rely on the hierarchical
    metadata mapping versus the volatility fallback. Bonds and funds with
    insufficient price history fall back to `BALANCED`.
    """
...
def _customer_band_distribution(
    ...
    Schema: `{"declared": {band -> count}, "predicted": {band -> count},
    "none_count": int}`. The `none_count` lives at the top level rather than as
    a fake band so the per-band dicts stay homogeneous.
    """
...
def _customer_self_discordance(
    ...
    """Per-customer fraction of profile-discordant transactions.

    Returns the array of per-customer discordant shares plus a summary dict.
    The array is the input to the histogram plotter; the summary feeds JSON.
    """
    valid = annotated_transactions.dropna(subset=["customer_band", "asset_band"])
...
def run_eda(
    ...
    """Compute all profile-coherence statistics and save figures.

    Returns the same `summary` dict that is written to disk so callers (e.g.
    a notebook wrapper) can render tables inline.
    """
